...
public class BlockGoldChest extends BlockContainer{
	...
	protected BlockGoldChest() {
		super(Material.rock);
		setCreativeTab(GoldenWorldsTabs.Blocks);
		setBlockTextureName("goldenworlds:goldenworlds_stone");
		setHardness(12.0f);
		setResistance(10.0f);
		setBlockName("chestGold");
		setStepSound(soundTypeGlass);
		setBlockBounds(0.0625F, 0.0F, 0.0625F, 0.9375F, 0.875F, 0.9375F);
	}
	
	public BlockGoldChest register(String name){
	...
	public void breakBlock(World world, int i, int j, int k, Block block, int par6) {
		TileEntityGoldChest items = (TileEntityGoldChest)world.getTileEntity(i, j, k);
		if(items != null) {
	...
	private IInventory getInventory(World world, int i, int j, int k) {
		Object obj = (TileEntityGoldChest)world.getTileEntity(i, j, k);
