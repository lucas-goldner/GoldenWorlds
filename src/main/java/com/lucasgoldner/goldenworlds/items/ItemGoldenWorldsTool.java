...
public class ItemGoldenWorldsTool extends GoldenWorldsModItem
{
    ...
    public ItemGoldenWorldsTool(float p_i45333_1_, ToolMaterial tool, Set p_i45333_3_, String tex, String name, boolean vanilla)
    {
    	super(tex, name);
        ...
        t = tool;
        if(!vanilla) setCreativeTab(GoldenWorldsTabs.Tools); else setCreativeTab(null);
        if (this instanceof ItemGoldenWorldsPickaxe)
        {
            toolClass = "pickaxe";
        }
        else if (this instanceof ItemGoldenWorldsAxe)
        {
            toolClass = "axe";
        }
        else if (this instanceof ItemGoldenWorldsShovel)
        {
            toolClass = "shovel";
        }
    }
    ...
    public void addInformation(ItemStack item, EntityPlayer player, List infoList, boolean par4) {
		...
		if (this instanceof ItemGoldenWorldsPickaxe) infoList.add(EnumChatFormatting.DARK_RED + "" + Math.round((t.getDamageVsEntity() + 4.0f)/4) + " Attack Damage");
		if (this instanceof ItemGoldenWorldsAxe) infoList.add(EnumChatFormatting.DARK_RED + "" + Math.round((t.getDamageVsEntity() + 4.0f)/4) + " Attack Damage");
		if (this instanceof ItemGoldenWorldsShovel) infoList.add(EnumChatFormatting.DARK_RED + "" + Math.round((t.getDamageVsEntity() + 4.0f)/5) + " Attack Damage");
    }
