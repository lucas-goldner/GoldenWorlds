...
public class ItemHermanoDeJuanesSpanwer extends Item {
    public ItemHermanoDeJuanesSpanwer(){
        setUnlocalizedName("Summon Hermano de Juanes");
    }
        ...
        else
        {
            ...
            else
            {
                if (movingobjectposition.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK)
                {
                    ...
                    EntityHermanoDeJuanes juanes = new EntityHermanoDeJuanes(world, i + 0.5, j + 1.0F,k + 0.5);

                    if (!world.getCollidingBoundingBoxes(juanes, juanes.boundingBox.expand(-0.1D, -0.1D, -0.1D)).isEmpty())
                    {
                        return itemStack;
                    }
                    ...
                    if (!world.isRemote)
                    {
                        world.spawnEntityInWorld(juanes);
                    }
